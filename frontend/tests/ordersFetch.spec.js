...
describe("orderSync.forceRefreshOrders", () => {
  ...
  it("throws on invalid parameters (missing userId)", async () => {
    const fetchFn = vi.fn();
    await expect(forceRefreshOrders(fetchFn, { userId: null })).rejects.toThrow(
  ...
  it("returns cached orders under fastMode within ~500ms", async () => {
    ...
    const result = await forceRefreshOrders(slowFetch, {
      userId: "u1",
      fastMode: true,
      showMessage: false,
    });
  ...
  it("applies only latest request results under concurrency (latest-only)", async () => {
    ...
    // 并发触发两次刷新
    const p1 = forceRefreshOrders(fastFirst, {
      userId: "u2",
      showMessage: false,
      fastMode: false,
    });
    const p2 = forceRefreshOrders(slowSecond, {
      userId: "u2",
      showMessage: false,
      fastMode: false,
    });
  ...
  it("propagates errors with clear messages", async () => {
    ...
    await expect(
      forceRefreshOrders(failingFetch, { userId: "u3" }),
    ).rejects.toThrow();
  });
});

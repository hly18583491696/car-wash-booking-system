...
describe("paymentFlow utils", () => {
  it("should show pay button for pending/confirmed and not paid", () => {
    expect(
      shouldShowPayButton({ status: "pending", paymentStatus: "pending" }),
    ).toBe(true);
    expect(
      shouldShowPayButton({ status: "confirmed", paymentStatus: "failed" }),
    ).toBe(true);
  });

  it("should hide pay button when paid or inappropriate status", () => {
    expect(
      shouldShowPayButton({ status: "completed", paymentStatus: "paid" }),
    ).toBe(false);
    expect(
      shouldShowPayButton({ status: "cancelled", paymentStatus: "pending" }),
    ).toBe(false);

        ...
        <div class="login-footer">
          ...
          <div class="quick-login">
            ...
            <div class="social-login">
              <el-button circle icon="ChatDotRound" @click="wechatLogin"
                >微信</el-button
              >
              <el-button circle icon="Message" @click="smsLogin"
                >短信</el-button
              >
            </div>
          </div>
      ...
      try {
        ...
        try {
          ...
          if (response.code === 200) {
            // 同步更新 userStore 和 localStorage
            userStore.setToken(response.data.token);
            userStore.setUserInfo(response.data.user);
            
            // 使用AuthManager处理登录（保持兼容性）
            ...
            if (loginSuccess) {
              ...
              ElMessage.success(
                `登录成功！欢迎，${response.data.user.realName || response.data.user.username}`,
              );

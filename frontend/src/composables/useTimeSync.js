...
export function useOrderSync(fetchOrders) {
  ...
  const updateChangedOrders = (serverOrders) => {
    serverOrders.forEach((serverOrder) => {
      ...
      if (
        !localOrder ||
        TimeUtils.toTimestamp(localOrder.updateTime) !==
          TimeUtils.toTimestamp(serverOrder.updateTime)
      ) {
        // 数据有变化，更新本地数据
        ...
        if (index !== -1) {
          orders.value[index] = {
            ...
            appointmentTime: TimeUtils.formatServerTime(
              serverOrder.appointmentTime,
            ),
          };
        } else {
          orders.value.push({
